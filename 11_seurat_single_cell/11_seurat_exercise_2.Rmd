...
colnames(seurat_after_qc@meta.data)
```
...
```{r}
#ncells is the cells number for subsampling, less is more time efficient 
#do not need to put UMI count to vars.to regress since SCT transfer already to this into account
seurat_after_qc <- SCTransform(seurat_after_qc,ncells = 5000,vars.to.regress = "percent_mt"
                               )

Assays(seurat_after_qc)
GetAssayData(seurat_after_qc,)
```
...
```{r}
view(seurat_after_qc[[]])
head(seurat_after_qc@assays$RNA)
head(seurat_after_qc@assays$SCT)
?sctransform::vst
#sctransform preform vst transgfer, the default is min cells=5 cells, so some genes have been filtered
```
...
Answer: yes, new UMI and features ,not raw counts, all saved in RNA assay
...
#ncount SCT and ncount RNA are different, SCT put normalize them into same level while maintain the biological difference
```
...
```{r}
seurat_after_qc %>% VariableFeatures(assay="RNA") %>% head(n=10)
seurat_after_qc %>% VariableFeatures(assay="SCT") %>% head(n=10)
which(seurat_after_qc %>% VariableFeatures(assay="RNA") ==
seurat_after_qc %>% VariableFeatures(assay="SCT") )
```
...
```{r}
seurat_after_qc <- RunUMAP(seurat_after_qc,assay = "SCT",dims = 1:20,reduction = "sct.pca",reduction.name="sct.umap")
```
...
```{r}
seurat_after_qc <-  FindNeighbors(seurat_after_qc,reduction="sct.pca",dims=1:20)
seurat_after_qc@graphs$SCT_snn
...
```{r}
hto12.umis <- readRDS(file = "/t1-data/project/obds/shared/resources/4_r_single_cell/seurat_day_2/data/hto12_umi_mtx.rds")
...
```{r}
hto12.htos <- readRDS(file = "/t1-data/project/obds/shared/resources/4_r_single_cell/seurat_day_2/data/hto12_hto_mtx.rds")
...
```{r}
hto12_object <-CreateAssayObject(hto12.umis.common,min.cells = 3,min.features = 200)

# Normalise with log normalization ,find variable genes and Scale RNA data
hto12_object<-NormalizeData()
