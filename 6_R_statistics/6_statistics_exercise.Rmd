...
```{r}
visual_data<-data.frame(value=x)
ggplot(visual_data,aes(x=value))+geom_histogram()
```
...
ggplot(visual_data,aes(x=value))+geom_histogram()+geom_vline(xintercept = mean(x),colour="red",lineype="solid")+geom_vline(xintercept = median(x),colour="red",lineype="dashed") +geom_vline(xintercept = mean(x)+sd(x)*c(-1,1),colour="blue",lineype="dashed") +geom_vline(xintercept = median(x)+ sd(x)*c(-1,1),colour="blue",lineype="solid")
...
visual_data<-data.frame(value=x)
ggplot(visual_data,aes(x=value))+geom_histogram(bins = 100)+
  geom_vline(xintercept = mean(x),colour="red",lineype="solid")+
  geom_vline(xintercept = median(x),colour="red",lineype="dashed") +
  geom_vline(xintercept = mean(x)+sd(x)*c(-1,1),colour="blue",lineype="dashed") +
  geom_vline(xintercept = median(x)+ sd(x)*c(-1,1),colour="blue",lineype="solid")
```
...
```{r}
logcounts_matrix <- read_excel(path =  "/project/obds/shared/resources/3_r_stats_genomics/statistics/data/GSE111543.xlsx",sheet = "logcounts")

dim(logcounts_matrix)
colnames(logcounts_matrix)
typeof(logcounts_matrix)
...
```{r}
cell_info <- read_excel(path =  "/project/obds/shared/resources/3_r_stats_genomics/statistics/data/GSE111543.xlsx",sheet = "cell_info")
all(cell_info$Sample == colnames(logcounts_matrix))
#check whether the IDs are same and in same order
head(cell_info)
```
...
gene_value<-logcounts_matrix[gene_index,]
gene_matrix<-data.frame(cell_info,value=gene_value)
gene_matrix<-select(gene_matrix,-"Sample")
unique(gene_matrix$Infection)

out <- t.test(value~Infection, gene_matrix)
out$p.value
str(out)
```
...
gene_value<-matrix[index,]
gene_matrix<-data.frame(metadata,value=gene_value)
gene_matrix<-select(gene_matrix,-"Sample")
unique(gene_matrix$Infection)
...
t_test_pvalues <- vapply(
  X =c(1:nrow(logcounts_matrix)),
  FUN = t_test_row,
  FUN.VALUE = numeric(1), matrix=logcounts_matrix,metadata=cell_info

)
# X = the row to choose to run the function, FUN =function, FUN.VALUE= the type of output , the rest part pass the parameter for the function
head(t_test_pvalues)
```
...
```{r}
gg_t<-data.frame(gene=rownames(logcounts_matrix),pvalue=t_test_pvalues)
head(gg_t)

gg_t %>% ggplot(mapping = aes(x=pvalue))+geom_histogram()
```
...
```{r}
result_bh <- p.adjust(gg_t$pvalue,method = "BH")
gg_t_2<-data.frame(gene=rownames(logcounts_matrix),pvalue=result_bh)
head(gg_t_2)

gg_t_2 %>% ggplot(mapping = aes(x=pvalue))+geom_histogram()
...
```{r}
gene_table <- read_excel( path =  "/project/obds/shared/resources/3_r_stats_genomics/statistics/data/GSE111543.xlsx",sheet = "gene_info"   )
head(gene_table)
gg_t_2[which.min(gg_t_2$pvalue),]

which(gene_table$gene_id=="ENSG00000131203")
...
```{r}
go_table <- read_excel( path =  "/project/obds/shared/resources/3_r_stats_genomics/statistics/data/GSE111543.xlsx",sheet = "go_db"    )
...
#find the DG from previous results, and group them into pathway and recalcuated across pathway
```{r}

##set the query as the gene list that is significant after adjusted
query <- gg_t_2$gene[gg_t_2$pvalue < 0.05]
query <- na.omit(query)
#remove NA
#universe is all gene name
universe <- rownames(logcounts_matrix)
#geneset is the Go list(1) gene that involved the this pathway
geneset <- go_list[[1]]
#build a table, test whether this genes in geneset or query, TRUE and FALSe set the factors order
cross_table <- data.frame(
  gene_id = universe,
  geneset = factor(universe %in% geneset, c(TRUE, FALSE)),
  query = factor(universe %in% query, c(TRUE, FALSE))
)
test_table <- table(DEgeneset=cross_table$geneset, DEgene=cross_table$query)
...
```{r}
fisher_test_pathway <- function(index,gg_t_2 ,logcounts_matrix   ) {
...
#universe is all gene name
universe <- rownames(logcounts_matrix)
#geneset is the Go list(index) gene that involved the this pathway
geneset <- go_list[[index]]
#build a table, test whether this genes in geneset or query, TRUE and FALSe set the factors order
cross_table <- data.frame(
  gene_id = universe,
  geneset = factor(universe %in% geneset, c(TRUE, FALSE)),
...
test_table <- table(DEgeneset=cross_table$geneset, DEgene=cross_table$query)
return(fisher.test(test_table)$p.value)


}
fisher_test_pathway(index = 3,gg_t_2 ,logcounts_matrix   )
```
