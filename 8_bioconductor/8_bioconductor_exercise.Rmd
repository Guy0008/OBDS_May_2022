...
```{r}
truseq_adapters <- readDNAStringSet(filepath = "/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/TruSeq3-PE-2.fa")
truseq_adapters
class(truseq_adapters)

file.edit("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/TruSeq3-PE-2.fa")
#file.edit() is a funciton to edit the file in txt form
```
...
```{r}
library(BSgenome.Hsapiens.UCSC.hg38.masked)# BSgenome is the master package and then install the daughter packages
```
...
# what is the unique name of sequence
```
...
```{r}
mcols(demo_granges)
#extract the metadata from this object
```
...
```{r}
gtf_ACTB$transcript_name %>% unique() %>% setdiff(NA) %>% length()
...
```{r}
gtf_ACTB_exons <- gtf_ACTB %>% subset(type=="exon")
gtf_ACTB_transcripts <- gtf_ACTB_exons %>% split(gtf_ACTB_exons$transcript_id)
...
```{r}
class(org.Hs.eg.db)
#organsie human eg database
#this is the offline reference dataset you can query for the gene
```
...
```{r}
getBM(c("hgnc_symbol","chromosome_name","start_position","end_position","strand"), 
      filters = "hgnc_symbol",
...
colData(demo_se)
colData(demo_se)$score<-c(23,567)
rowData(demo_se)
mcols(demo_se)
metadata(demo_se)
```
...
```{r}
assay_counts <- read_csv(file = "/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/summarized_experiment/counts.csv")
...
```{r}
sample_metadata <- read_csv(file = "/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/summarized_experiment/sample_metadata.csv")
sample_metadata %>% column_to_rownames(var="...1")->sample_metadata
head(sample_metadata)
```

```{r}
gene_metadata <- read_csv(file = "/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/summarized_experiment/gene_metadata.csv")
gene_metadata %>% column_to_rownames(var="...1")->gene_metadata
head(gene_metadata)
dim(gene_metadata)
```
...
demo_se
#when merging, colname of assays need to be same as rowData colnames
```
...
```{r}
demo_deseq <- as( demo_se,Class = "DESeqDataSet"  )
demo_deseq
```
